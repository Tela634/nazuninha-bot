...
const { reportError, youtube, tiktok, pinterest, igdl, sendSticker, FilmesDL, styleText, emojiMix, upload, mcPlugin, tictactoe, rpg, consulta, toolsJson, vabJson, ai }  = require(__dirname+'/funcs/exports.js');
...
break;
   ...
   nazu.react('🔎');
   f1 = await consulta(q, 'ABREVIADO', 'nome');
   ...
   nazu.react('🔎');
   f1 = await consulta(q, 'CREDILINK', 'cpf');
   if(f1.error) return reply(f1.error);
   ...
   if(!q) return reply(`🔍 Está faltando o cpf.\n\nExemplo: ${prefix}${command} 01234567890`);
   nazu.react('🔎');
   f1 = await consulta(q, 'COMPLETA', 'cpf');
   if(f1.error) return reply(f1.error);
