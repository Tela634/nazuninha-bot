// Sistema de download e pesquisa tiktok
...
async function tiktok(url) { try { res = await axios.get(`https://www.tikwm.com/api/?url=${url}`); obj = { ok: true, criador: 'Hiudy' }; if (res.data?.data?.music_info?.play) obj.audio = res.data.data.music_info.play; if (res.data?.data?.images) { obj.type = 'image'; obj.mime = ""; obj.urls = res.data.data.images; } else { obj.type = 'video'; obj.mime = 'video/mp4'; obj.urls = [res.data.data.play]; } if (res.data?.data?.title) obj.title = res.data.data.title; return obj; } catch (e) { return { ok: false, msg: 'Ocorreu um erro ao realizar o download' }; } }
